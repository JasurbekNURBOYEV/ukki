...
class DataBase:
	...
	def store_request(self, agent=None, path=None, user_id=None, ip=None, 
		device_name=None, device_id=None, token=None):
		self.executor(models.Request.objects.create, 
				agent=agent, path=path, user_id=user_id,
				ip=ip, device_name=device_name, device_id=device_id,
				token=token
			)
	...
	def get_temp_acc(self, tg_id, delete=False):
		acc_info = self.executor(models.TemporaryAccount.objects.get, tg_id=tg_id)
		if acc_info != None:
			data = {
				'login': acc_info.login,
				'password': acc_info.password,
				'tg_id': acc_info.tg_id
			}
			if delete:
				acc_info.delete()
		else:
	...
	def register_device(self, user_id, device_id, device_name):
		...
		if existing_device != None:
			existing_device.user_id = self.executor(models.Account.objects.get, user_id=user_id)
	...
	def terminate_old_sessions(self, user_id, tg_id):
		...
		existing_tg_accs = self.executor(models.TelegramAccount.filter, user_id=user_id)
		terminated_tg_accs = []
	...
	def register(self, tg_id, freshman_year=2018, name=None, login=None, password=None):
		...
		token = self.executor(models.Account.objects.get, login=login)
		return terminated_tg_accs, token.token if token != None else None 
	...
	def get_friends(self, user_id):
		data = self.executor(models.Subject.objects.filter, user_id=user_id)
		data = {'all_friends': {}}
		friends = set()
		if len(data) > 0:
			for subject in data:
				for stream in subject.streams.split('###'):
					friends_for_this_stream = self.executor(
						...
						streams__contains=f'.{stream}.').exclude(
						user_id=user_id).ordered_by('name')
					data[stream] = [x.user_id for x in friends_for_this_stream]
					friends.add(data[stream])
	...
	def increment_stream_members_count(self, name):
		...
		if existing_stream != None:
			...
			existing_stream.save()
			print("Saved: {} = {}".format(name, existing_stream.members_count))
	...
	def get_session(self, token):
		existing_account = self.executor(models.Account.objects.get, token=token)
		if existing_account != None:
			existing_session = self.executor(models.Session.objects.get, user_id=existing_account.user_id)
			if existing_session != None:
				return {'session': existing_session, 'account': existing_account}
			else:
				return {'session': None, 'account': existing_account}
		return None

	def store_session(self, token, xsrf_token, tuit_lms_session):
		existing_account = self.executor(models.Account.objects.get, token=token)
		if existing_account != None:
			existing_session = self.executor(models.Session.objects.get, user_id=existing_account)
			if existing_session != None:
				existing_session.xsrf_token = xsrf_token
				existing_session.tuit_lms_session = tuit_lms_session
				existing_session.save()
			else:
				self.executor(models.Session.objects.create,
					xsrf_token=xsrf_token,
					tuit_lms_session=tuit_lms_session,
					user_id=existing_account)
